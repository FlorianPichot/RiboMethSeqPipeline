...
names (results_max) [2] <- paste0 ("max.", z)
...
n = length (score_max$position)
score_max$max_rank <- 1:n
score_max$type <- NULL
score_max$identity <- NULL
scoreA$scoreA_rank <- 1:n
scoreA$type <- NULL
scoreA$identity <- NULL


graphe <- merge (score_max, scoreA, by = "position")
...
ggsave (filename = paste0 ("Histogramme_global_score_max_6_", y, "_", z, ".pdf"), plot = plotmax1)
...
ggsave (filename = paste0 ("Histogramme_colour_score_max_6_", y, "_", z, ".pdf"), plot = plotmax2)

windows()
plotA1 <- ggplot (graphe, aes(scoreA)) + xlim(0,1) + geom_histogram(alpha = 0.3, fill = "green", bins = 250) + theme_bw()
ggsave (filename = paste0 ("Histogramme_global_score_A_6_", y, "_", z, ".pdf"), plot = plotA1)
...
ggsave (filename = paste0 ("Histogramme_colour_score_A_6_", y, "_", z, ".pdf"), plot = plotA2)
...
theme_bw()
ggsave (filename = paste0 ("Density_score_max_6_", y, "_", z, ".pdf"), plot = last_plot())
...
geom_density(data = Psi, alpha = 0.5, fill = "red", colour = "red") +
theme_bw()
ggsave (filename = paste0 ("Density_score_A_6_", y, "_", z, ".pdf"), plot = last_plot())
...
ggsave (filename = paste0 ("Density+plot_rank_6_", y, "_", z, ".pdf"), plot = grid.arrange(arrangeGrob(p2,ncol=2,widths=c(3,1)),
             arrangeGrob(p1,p3,ncol=2,widths=c(3,1)),
             heights=c(1,3)))
...
ggsave (filename = paste0 ("Density+plot_6_score_", y, "_", z, ".pdf"), plot = grid.arrange(arrangeGrob(p2,ncol=2,widths=c(3,1)),
             arrangeGrob(p1,p3,ncol=2,widths=c(3,1)),
             heights=c(1,3)))
